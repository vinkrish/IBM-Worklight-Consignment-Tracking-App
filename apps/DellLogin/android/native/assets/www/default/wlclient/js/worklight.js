...
this.setLocalization = function(){
    	...
    	//var preferLocale = WL.Client.getUserPref('preferLocale');
    	/*if(preferLocale != undefined){
    		//To handle cases where the application developer passses locale in lang-countrycode format instead of 
        	//lang_countrycode format
    		if(preferLocale.search("-")>0){
        		preferLocale=preferLocale.replace("-","_");
        	}
    		WL.ClientMessages = window["WLClientMessages_"+preferLocale];
    		if(WL.ClientMessages == undefined){
        		var loc= preferLocale.search("_");
        		var preferlang = preferLocale.substr(0,loc);
        		WL.ClientMessages = window["WLClientMessages_"+ preferlang];
        	}    		
    	}else if(preferLocale == undefined){
    	*/	//If developer or user has not set a preferred locale, use the device locale to select languages.  		
    		
       //To handle cases where WL.App.getDeviceLocale() returns lang-countrycode 
       //instead of lang_countrycode.
	   // Blackberry returns language code for WL.App.getDeviceLocale()	
	  
		var deviceLocale = WL.App.getDeviceLocale();    	
		UserVisibleMessages = WL.ClientMessages ;
		
    	if(deviceLocale.search("-")>0){
        	deviceLocale=deviceLocale.replace("-","_");
       	}
    	
    	 // Windows 8 returns country code for WL.App.getDeviceLocale()
    	
    	if ( WL.Client.getEnvironment() == WL.Env.WINDOWS_PHONE_8){    		  
    		var winLocale = navigator.userLanguage;
    		if(winLocale.search("-")>0){
    		  winLocale=winLocale.replace("-","_");    		   
    		}
    		WL.ClientMessages = window["WLClientMessages_"+winLocale];
    	}else{
    		WL.ClientMessages = window["WLClientMessages_"+deviceLocale];
    	}
    	    	
    	if(WL.ClientMessages == undefined){
    		    		
        	WL.ClientMessages = window["WLClientMessages_"+WL.App.getDeviceLanguage()];
        }
    	
    	if(WL.ClientMessages == undefined){
    		
    		WL.ClientMessages = window["WLClientMessages_en_US"];
    	}
    	if(WL.ClientMessages == undefined){
    		    		
    		WL.ClientMessages = window["WLClientMessages_en"];
    	}	
    	
    	//if none of the above options are available, use the original set of messages
    	if(WL.ClientMessages == undefined){
    		    		
    		WL.ClientMessages = UserVisibleMessages;
    	}
        ...
        if (WL.EnvProfile.isEnabled(WL.EPField.USES_CORDOVA)) {
            ...
            if (WL.StaticAppProps.ENVIRONMENT == WL.Env.WINDOWS_PHONE || WL.StaticAppProps.ENVIRONMENT == WL.Env.WINDOWS_PHONE_8 || isAndroidDirectUpdateCase) {
            	...
            	cordova.exec(function (result) { WL.SimpleDialog.__callback(result); }, 
            			...
            			"WLCustomDialog", dialogPluginFunctionName, [text, title, buttonsArray]);
            } else {
