...
function EventTransmitter() {
	...
	this.flushBuffer = function() {
		...
		if (self.transmissionChunkQueue.length>0) {
			...
			{	
				WL.Logger.debug("Client is disconnected,connecting...");
				WL.Client.connect({
								onSuccess: function ()
									{
										WL.Logger.debug("Client is connected,trying to transmit...");
	...
	this.stopTransmission = function() {
		if (self.timeoutID != null) {
			clearTimeout(self.timeoutID);
			if (!self.transmitting) { // if not waiting for onsuccess / onfailure - try to flush immediately
				self.flushBuffer();
			}
			self.timeoutID = null;
			self.lastFlush = null;
